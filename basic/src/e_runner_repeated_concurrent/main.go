// create module
// > cd basic\src\e_runner_repeated_concurrent
...
func main() {
	...
	for {
		fmt.Println("\n--- Starting a new concurrent stop attempt ---")
		...
		// Launch the first goroutine to stop "examplesvca".
		go stopService(&wg, "wuauserv")

		// Launch the second goroutine to stop "examplesvcb".
		...
		fmt.Println("Waiting second(s) before the next attempt...")
